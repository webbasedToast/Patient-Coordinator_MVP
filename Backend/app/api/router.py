...
def create_transport_endpoint(data: TransportCreate):
    return create_transport_request(data.pickup_location, data.drop_off_location, data.assigned_timeframe)

@router.patch("/{transport_id}/status")
def update_transport(transport_id: int, update: TransportStatusUpdate):
    transport = update_transport_request() # TODO: Params?
    if not transport:
        raise HTTPException(status_code=404, detail=f"no transport found with id {transport_id}")
    return transport
